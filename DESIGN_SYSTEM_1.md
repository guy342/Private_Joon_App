...
| Last Covered row | `12px 16px` | `display: flex; align-items: center; gap: 12px; align-self: stretch`. Height is content-driven (no fixed height). **Three flat children, all left-packed**: name (default `flex: 0 1 auto` + `min-width: 0` + ellipsis on overflow), Deployed pill (`flex-shrink: 0`), date (`flex-shrink: 0`, content-sized). No `flex: 1` on the name — items hug the left and the date sits 12px after the pill (so the pill's right edge varies row-to-row with name length). Earlier iteration used `flex: 1` on name + `width: 48` on the date to vertically align the pill column across rows; retired 2026-05-08 — the user wanted the date packed against the pill, not pushed to the right. |
...
**Cursor-tracking spotlight (hover) — tier cells only:** cells **with a tier** carry a soft radial glow that follows the cursor while hovering. The cell is `position: relative; overflow: hidden`; mouse `clientX/Y` minus the cell's `getBoundingClientRect()` gets written to the cell's own `--spot-x` / `--spot-y` CSS variables via `el.style.setProperty` inside `onMouseMove` — direct DOM writes, no React re-render per move. A sibling overlay (`position: absolute; inset: 0; pointer-events: none; mix-blend-mode: plus-lighter`) renders `radial-gradient(140px circle at var(--spot-x) var(--spot-y), color-mix(in oklab, <tier.bar> 28%, transparent), transparent 65%)`. The overlay's `opacity` toggles between 0 and 1 via a `useState` hover flag with a 180ms ease-out transition, so enter/leave fades but cursor motion stays GPU-cheap. **Empty cells (`tier === null`) skip the spotlight entirely** — the placeholder represents "no detection at this technique" and is non-interactive, so there's nothing to highlight. Implement by early-returning the empty variant before the handlers + overlay are wired up; don't gate the overlay opacity, gate the whole interactive shell. **`mix-blend-mode: plus-lighter`** is what makes the glow look like light: it adds the gradient color to whatever is beneath, brightening both the bg and the text inside the spot rather than overlaying them. Don't drop the blend mode for a flat `rgba` fill — you'll get a muddy disk instead of a torch.
